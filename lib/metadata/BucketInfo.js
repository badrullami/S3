...
export default class BucketInfo {
    ...
    * @param {string[]} [cors[].allowedHeaders] - headers allowed in a OPTIONS
    * request via the Access-Control-Request-Headers header
    * @param {number} [maxAgeSeconds] - seconds browsers should cache OPTIONS
    * response
    ...
    */
    constructor(name, owner, ownerDisplayName, creationDate,
        ...
        if (cors) {
            ...
            const { id, allowedMethods, allowedOrigins, allowedHeaders,
            maxAgeSeconds, exposeHeaders } = cors;
            assert(id === undefined || typeof id === 'string');
            assert(Array.isArray(allowedMethods));
            assert(Array.isArray(allowedOrigins));
            assert(allowedHeaders === undefined ||
                Array.isArray(allowedHeaders));
            assert(maxAgeSeconds === undefined ||
                typeof maxAgeSeconds === 'number');
            assert(exposeHeaders === undefined || Array.isArray(exposeHeaders));
        }
    ...
    */
    ...
    static deSerialize(stringBucket) {
        ...
        return new BucketInfo(obj.name, obj.owner, obj.ownerDisplayName,
            ...
            obj.versioningConfiguration, websiteConfig, obj.cors);
    }
    ...
    */
     ...
     * @param {string[]} cors[].allowedMethods - methods allowed for CORS
     * @param {string[]} cors[].allowedOrigins - origins allowed for CORS
     * @param {string[]} [cors[].allowedHeaders] - headers allowed in a OPTIONS
     * request via the Access-Control-Request-Headers header
     * @param {number} [maxAgeSeconds] - seconds browsers should cache OPTIONS
     * response
